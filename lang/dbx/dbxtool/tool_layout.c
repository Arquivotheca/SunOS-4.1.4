#ifndef lint
static	char sccsid[] = "@(#)tool_layout.c 1.1 94/10/31 Copyr 1987 Sun Micro";
#endif
 ...
 *
 * NOTE: Clients of sunwindows are supposed to be able to provide their own
 ...
 *
 * This tool_layoutsubwindows is specific to dbxtool.
 */
	...
	struct	tool *tool;
{
	...
	struct tool * bf = (struct tool *)get_base_frame( );
	...
	{ extern Rect tool_rect;

	    stretch(rect.r_width-tool_rect.r_width,
		rect.r_height-tool_rect.r_height);
	}
...
static tool_lsw_die ( ) {
	fprintf( stderr, "dbxtool cannot get rect for base_frame\n" );
	exit( 1 );
}
