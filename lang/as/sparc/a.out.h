...
/* Do not use these two macros in new code; they are here only for
 * purposes of backward compatibility.
 */
#define	OLD_PAGSIZ	0x0800	/* Page size under Release 2.0		*/
#define	OLD_SEGSIZ	0x8000	/* Segment size under Release 2.0	*/
...
#define	N_SYMOFF(x)   /* symbol table */	\
	(N_DRELOFF(x) + (x).a_drsize)

#define	N_STROFF(x)   /* string table */	\
	(N_SYMOFF(x) + (x).a_syms)
...
struct reloc_info_68k		/* used when header.a_machtype == M_680[12]0  */
{
...
unsigned int	r_symbolnum:24,	/* local symbol ordinal */
		...
		:4;		/* unused */
/* (addend for 68k relocation values is found in the object code itself) */
};
...
/* If (r_extern == TRUE), then r_index is an index into the symbol structures
 * in the file.  If it r_extern is FALSE, then r_index is not an index, but
 * is a segment # (i.e. of type "Segment", defined elsewhere in this file).
 */
