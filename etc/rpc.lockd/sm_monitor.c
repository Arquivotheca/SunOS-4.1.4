#ifndef lint
static char sccsid[] = "@(#)sm_monitor.c 1.1 94/10/31 SMI";
#endif
...
struct stat_res *
stat_mon(sitename, svrname, rsys, my_prog, my_vers, my_proc, func, len, priv)
	char *sitename;
	char *svrname;
        long rsys;
	...
	char *priv;
{
	...
	if (sitename != NULL) {
                if ((hp = gethostbyname(sitename)) == NULL) {
                        if ((hp = gethostbyaddr(&rsys,sizeof(long), AF_INET)) != NULL)          
                                sitename = hp->h_name;
                }
	...
	mon_retries = 0;
again:
	if ((rpc_err = call_udp(svr, rsys , SM_PROG, SM_VERS, func, xdr_argument, ip,
...
cancel_mon()
{
	...
	resp = stat_mon(NULL, hostname, 0, PRIV_PROG, PRIV_VERS, PRIV_CRASH, SM_UNMON_ALL, NULL, NULL);
	if (resp->res_stat == stat_fail)
		return;
        resp = stat_mon(NULL, hostname, 0, PRIV_PROG, PRIV_VERS, PRIV_RECOVERY, SM_UNMON_ALL, NULL, NULL);
	resp = stat_mon(NULL, NULL, 0, 0, 0, 0, SM_SIMU_CRASH, NULL, NULL);
	if (resp->res_stat == stat_fail)
		return;
