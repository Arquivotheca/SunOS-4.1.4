#ifndef lint
static	char sccsid[] = "@(#)simdisk.c 1.1 94/10/31";
#endif
...
struct simdisk {
	...
	0, 0, SIMD_RSIZE, SIMD_RDEV,
	0, 0, SIMD_SSIZE, SIMD_SDEV,
	0, 0, SIMD_USIZE, SIMD_UDEV
};
	...
	register int flag;
{
	...
	if ((simdunits[unit].fdes = s_open(name, O_RDWR)) == -1 ) {
	...
	register struct buf *bp;
{
	...
	if (s_lseek(fdes, bn << DEV_BSHIFT, L_SET) == -1) {
		/* avoid a problem in sas by trying the seek once more */
		if (s_lseek(fdes, bn << DEV_BSHIFT, L_SET) == -1) {
