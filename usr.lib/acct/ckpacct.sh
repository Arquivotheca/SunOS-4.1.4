...
#ident	"@(#)ckpacct.sh 1.1 94/10/31 SMI" /* from S5R3 acct:ckpacct.sh 1.4 */
...
_blocks=`df ${_adm}| sed '/ /!{N;s/\n/ /;}' | awk '{print \$4}' | egrep "[0-9]"`
if [ "$_blocks" -lt $_MIN_BLKS   -a  -f /tmp/acctoff ];then
	echo "	ckpacct: $_adm still low on space ($_blocks blks);"
	echo "	acctg still off"
	( echo "	ckpacct: $_adm still low on space ($_blocks blks);"
	echo "	acctg still off" ) | mail root 
	exit 1
elif [ "$_blocks" -lt $_MIN_BLKS ];then
	echo "	ckpacct: $_adm too low on space ($_blocks blks);"
