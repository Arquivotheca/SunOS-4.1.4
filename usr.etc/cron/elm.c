...
#ifndef lint
static	char sccsid[] = "@(#)elm.c 1.1 94/10/31 SMI"; /* from S5R3 1.2 */
#endif
...
/**************************/
void el_add(event,time,id,firstruntime)
/**************************/
int	*event, id;
time_t	time;
time_t        firstruntime;
...
/* add works slightly differently than in the reference.  if the
	...
	if (being_rescheduled) {
		while ((n2->time == time) && (n2->wastorun > firstruntime)) {
			n2 = n2->left;
		}
	}
	n->right = n2->right;
	...
	if (being_rescheduled) {
		being_rescheduled = 0;
		if ( (current->time == time) && (current->wastorun > firstruntime)) current = n;
	}
}
