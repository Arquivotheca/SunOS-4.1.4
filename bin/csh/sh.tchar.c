...
#ifndef lint
static char *sccsid = "@(#)sh.tchar.c 1.1 94/10/31 SMI.";
#endif
/*
     ...
     int	nchreq;	/* # of tchars requrested. */
{
	...
	while(nchread<nchreq){
		...
		{
			int flow_flags; /* flow control flags */

			/* make blocking - if non-blocking */
			if (( flow_flags = fcntl (d, F_GETFL, 0)) & O_NDELAY) {
				fcntl (d, F_SETFL, flow_flags & ~O_NDELAY);
			}
		}
		k=read(d, p, m);
	...
	assert(nchreq<=BUFSIZ);
	{
		int flow_flag; /* flow control flags */

		/* make blocking - if non-blocking */
		if (( flow_flag = fcntl (d, F_GETFL, 0)) & O_NDELAY) {
			fcntl (d, F_SETFL, flow_flag & ~O_NDELAY);
		}
	}
